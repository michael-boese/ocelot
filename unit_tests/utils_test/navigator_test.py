...
def compare_p_array(p_array, p_array_ground_truth):
    ...
    assert np.allclose(p_array.E, p_array_ground_truth.E, rtol=1e-05, atol=1e-08, equal_nan=False)
...
def get_maps_from_single_element_lat_test(p_array_init, element, method):
    ...
    for tms, _, _, _ in navi.get_next_step():
        for t_map in tms:
            t_map.apply(p_array)
    p_array_ground_truth = load_particle_array("unit_tests/test_data/ground_truth_SBend_output.npz")
...
def test_maps_of_fel():
    ...
    for tms, _, _, _ in navi.get_next_step():
        ...
        for t_map in tms:
            t_map.apply(p_array)
            data_name = f"unit_tests/test_data/ground_truth_output_{name}_{gt_i+2}.npz"
            p_array_ground_truth = load_particle_array(data_name)
            compare_p_array(p_array, p_array_ground_truth)
            i += 1
            gt_i += 1
